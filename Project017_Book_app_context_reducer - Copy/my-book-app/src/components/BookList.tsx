...
const BookList = (props:IProps) => {
    ...
    const gettingData = async() => {
        ...
        .then(data => {
            ...
            setGotData(data);
            items.dispatch({type : 'BOOK_LIST', payload : data})
        })
    }
    ...
    useEffect(() => {
        gettingData();
        // console.log('items'+items.books.book);
    })
    ...
    const SearchBodyHandler = ()=> {
        const data = items.books.book;        //JSON.parse(records!);
        var searchedRecord = [];
        console.log(option);
        console.log(value);
        if(option === 'title'){
